...
describe("Limitrr object test", () => {
  ...
  it("Set empty route to correct default properties", done => {
    ...
    expect(routes.empty).to.have.keys([
      "requestsPerExpiry",
    ...
    expect(routes.empty.errorMsgs.requests).to.equal(
      "As you have made too many requests, you are being rate limited."
    );
    expect(routes.empty.errorMsgs.completed).to.equal(
...
describe("get() and reset()", () => {
  it("Should get correct values", () => {
    return limitrr.get("test", "two").then(res => {
      //Testing compatibility with old way of passing in values to function

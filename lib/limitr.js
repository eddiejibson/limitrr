 ...
 * @Last Modified Time: November 3rd 2018, 7:47:41 pm
...
class limitr {
    ...
    complete(discriminator, route = "default") {
        return new Promise((resolve, reject) => {
            this.db.multi()
                .incr(`limitr:${this.options.keyName}:${discriminator}:${route}:completed`)
                .expire(`limitr:${this.options.keyName}:${discriminator}:${route}:completed`, this.options.completedExpiry).exec().then((result) => {
                    if (result[1][1]) {
                        resolve(true);
                    } else {
                        resolve(false);
    ...
    get(discriminator, type = "completed", route = "default") {
        return new Promise((resolve, reject) => {
            this.db.get(`limitr:${this.options.keyName}:${discriminator}:${route}:${type}`).then((result) => {
                resolve(result);
            }).catch((err) => {
                reject(err);
            });
    ...
    reset(discriminator, type = "completed", route = "default") {
        return new Promise((resolve, reject) => {
            this.db.del(`limitr:${this.options.keyName}:${discriminator}:${route}:${type}`).then((result) => {
                resolve(result);
            }).catch((err) => {
                reject(err);
            });
    ...
    resetAll(discriminator, route = "default") {
        return new Promise((resolve, reject) => {
            this.db.multi()
                .del(`limitr:${this.options.keyName}:${discriminator}:${route}:completed`)
                .del(`limitr:${this.options.keyName}:${discriminator}:${route}:requests`).then((result) => {
                    resolve(result);
                }).catch((err) => {
                    reject(err);
                });

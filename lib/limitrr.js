...
class limitrr {
    ...
    limit(obj = {
        ...
        return async (req, res, next) => {
            ...
            if (!test && !this.routes[route].limitLocal && (discriminator === "127.0.0.1" || discriminator === "::1")) return next();
            ...
            if (parseInt(result[0][1]) >= this.routes[route].requestsPerExpiry) {
                ...
                let incrResult = await this.db.multi().incr(`${key}:requests`).expire(`${key}:requests`, this.routes[route].expiry).exec().catch((err) => {
                    next(err);
                });

 ...
 * @Last Modified Time: November 11th 2018, 10:53:23 pm
...
class limitrr {
    constructor(conf) {
        ...
        conf.routes.default = {
            ...
            "errorMsgs": {
                ...
                "completed": conf.routes.errorMsgs.completed || errorMsgs.requests || conf.options.errorMsg || "As you performed too many successful actions, you are being rate limited."
            }
        };
        this.options = {
        ...
        return (req, res, next) => {
            ...
            this.db.multi()
                ...
                .then(result => {
                    ...
                    } else if (result[1][1] >= (this.routes[route].completedActionsPerExpiry || this.routes.default.requestsPerExpiry)) {
                        return res.status(this.options.errorStatusCode).json({
    ...
    get(discriminator, route = "default") {
        return new Promise((resolve, reject) => {
            type = false; //This used to be passed into the function. It's coming back, just not like before. This is used as a placeholder.
            if (!type) {
                this.db.multi()

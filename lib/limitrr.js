...
class limitrr {
    ...
    limit(obj = {
        ...
        return async (req, res, next) => {
            ...
            res.header("X-RateLimit-Remaining", (this.routes[route].requestsPerExpiry - res[0][1] - 1));
            res.header("X-RateLimit-Reset", parseInt(res[2][1]));
            res.header("X-RateLimit-Limit-Actions", this.routes[route].completedActionsPerExpiry);
            res.header("X-RateLimit-Remaining-Actions", (this.routes[route].completedActionsPerExpiry - res[1][1]));
            res.header("X-RateLimit-Reset-Actions", parseInt(res[3][1]));
            if (parseInt(res[0][1]) >= this.routes[route].requestsPerExpiry) {
                return res.status(this.options.errorStatusCode).json({
                    error: this.routes[route].errorMsgs.requests
                });
            } else if (parseInt(res[1][1]) >= this.routes[route].completedActionsPerExpiry) {
                return res.status(this.options.errorStatusCode).json({
                    error: this.routes[route].errorMsgs.completed
                });
            } else if (res[0][1]) {
                let res = await this.db.incr(`${key}:requests`).catch((err) => {
                    next(err);
                });
                if (res > 0) {
                    next();
                ...
                let res = await this.db.multi().incr(`${key}:requests`).expire(`${key}:requests`, this.routes[route].expiry).exec().catch((err) => {
                    next(err);
                });
                if (res.length > 0) {
                    next();

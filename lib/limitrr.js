 ...
 * @Last Modified Time: November 7th 2018, 3:44:02 pm
...
class limitrr {
    ...
    limit(route = "default") {
        return (req, res, next) => {
            ...
            this.db.multi()
                ...
                .exec()
                .then(result => {
                    //Also get competed actions
                    if (result[0][1] >= conf.requestsPerExpiry || result[1][1] >= conf.completedActionsPerExpiry) {
    ...
    get(discriminator, type = false, route = "default") {
        return new Promise((resolve, reject) => {
            if (!type) {
                this.db.multi()
    ...
    reset(discriminator, type = "completed", route = "default") {
        return new Promise((resolve, reject) => {
            this.db
                .del(
                    `limitrr:${this.options.keyName}:${discriminator}:${route}:${type}`
                )
                .then(result => {
                    resolve(result);
                })
    ...
    resetAll(discriminator, route = "default") {
        return new Promise((resolve, reject) => {
            this.db
                ...
                .del(
                    `limitrr:${this.options.keyName}:${discriminator}:${route}:requests`
                )
                .then(result => {
                    resolve(result);
                })

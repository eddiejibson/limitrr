 ...
 * @Last Modified Time: November 16th 2018, 5:15:38 pm
...
class limitrr {
    constructor(conf) {
        ...
        this._setDefaultToUndefined(this.routes, this.routes.default)
        this.db = new Redis({
        ...
        return (req, res, next) => {
            ...
            this.db.multi()
                ...
                .then((result) => {
                    ...
                    res.header("X-RateLimit-Remaining", (this.routes[route].requestsPerExpiry - result[0][1]));
                    ...
                    if (result[0][1] >= this.routes[route].requestsPerExpiry) {
                        return res.status(this.options.errorStatusCode).json({
                            error: this.routes[route].errorMsgs.requests
                        });
                    } else if (result[1][1] >= this.routes[route].completedActionsPerExpiry) {
                        return res.status(this.options.errorStatusCode).json({
    ...
    _setDefaultToUndefined(obj, defaultObj) {
        return new Promise((resolve, reject) => {
            Object.keys(obj).forEach((obj2, index) => {
                obj2 = obj[obj2];
                Object.keys(this.routes.default).forEach((key, index) => {
                    if (!obj2[key]) {

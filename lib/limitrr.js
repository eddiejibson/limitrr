 ...
 * @Last Modified Time: December 20th 2018, 3:09:33 pm
...
class limitrr {
    ...
    complete(obj, o_route = "default") {
        return new Promise((resolve, reject) => {
            ...
            this.db.get(`limitrr:${this.options.keyName}:${discriminator}:${route}:completed`).then((result) => {
                if (result <= 0) {
                    this.db.multi()
                        .incr(
                    ...
                    this.db.incr(`limitrr:${this.options.keyName}:${discriminator}:${route}:completed`).then((result) => {
                        if (result > 0) {
                            resolve(result);
                        } else {
                            reject("Keystore returned invalid data")
